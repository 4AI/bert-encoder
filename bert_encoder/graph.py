...
from bert_encoder.bert import modeling
...
def optimize_bert_graph(args, logger=None):
    if not logger:
        logger = set_logger('GRAPHOPT', args.verbose)
    try:
        pb_file = os.path.join(args.model_dir, 'bert_model.pb')
        ...
        init_checkpoint = os.path.join(args.model_dir, args.ckpt_name)
        ...
        logger.info('write graph to a tmp file: %s' % args.model_dir)
